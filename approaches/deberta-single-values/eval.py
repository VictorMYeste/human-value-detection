...
gold_labels_path = "../../data/test-english/labels.tsv"
...
# Combine attained and constrained for gold labels
for label in labels:
    gold_labels[label + "_gold"] = gold_labels[[label + " attained", label + " constrained"]].max(axis=1)

# Combine attained and constrained for predictions
for label in labels:
    predictions[label + "_pred"] = predictions[[label + " attained", label + " constrained"]].max(axis=1)

# Merge the datasets on Text-ID and Sentence-ID
merged = pd.merge(gold_labels, predictions, on=["Text-ID", "Sentence-ID"])
