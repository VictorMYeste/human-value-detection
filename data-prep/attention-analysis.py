...
def get_attributions(sentence):
    ...
    attributions = ig.attribute(
        input_embeds,
        target=0,  # Compute attributions for predicted class
    ...
    clean_tokens = [token.replace("Ġ", " ").strip() for token in tokens]
...
top_presence_tokens = sorted(mean_presence_attributions.items(), key=lambda x: x[1], reverse=True)[:100]
top_absence_tokens = sorted(mean_absence_attributions.items(), key=lambda x: x[1], reverse=True)[:100]
...
# Save presence tokens to a text file
presence_df.to_csv("files/top_presence.txt", sep="\t", index=False, header=True)
absence_df.to_csv("files/top_absence.txt", sep="\t", index=False, header=True)
print("Top 100 presence and absence tokens saved as 'top_100_presence.txt' and 'top_100_absence.txt'.")
...
plt.ylabel("Token")
plt.tight_layout()
strFile = "files/attribution_presence_1.png"
...
plt.ylabel("Token")
plt.tight_layout()
strFile = "files/attribution_presence_0.png"
if os.path.isfile(strFile):
   os.unlink(strFile)
plt.savefig(strFile, dpi=300)
plt.close()
